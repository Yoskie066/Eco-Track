...
const ReportedWaste = () => {
  ...
  useEffect(() => {
    const getAllReportedWasteData = () => {
      const allData = [];
      
      // Get data from all user-specific keys (reportedWaste_userEmail)
    ...
    const rawData = getAllReportedWasteData();
    ...
    const sortedData = formatted.sort((a, b) => {
      const dateA = new Date(a.dateReported || 0);
      const dateB = new Date(b.dateReported || 0);
      return dateB.getTime() - dateA.getTime();
    });
    
    setReportedData(sortedData);
    setTimeout(() => setIsLoaded(true), 300);
  }, []);

  const filteredData = reportedData.filter((item) =>
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8 lg:p-12 xl:p-20">
      <div className="text-center mb-4 md:mb-8 overflow-hidden">
        ...
        >
          Display all reported waste records from users within Eco-Track Web Application.
        </motion.p>
      </div>
            ...
            <div className="overflow-x-auto">
                    ...
                    >
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-gray-700 font-normal">{item.colorInput || 'N/A'}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-gray-700 font-normal">{item.dateReported || 'N/A'}</span>
                        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200 hidden md:table">
                <thead className="bg-green-600">
                  <tr>
                    {["Email", "Waste Name", "Category", "Sub-category", "Color", "Location", "Date Reported", "Description", "Image"].map((header) => (
                      <th
                        key={header}
